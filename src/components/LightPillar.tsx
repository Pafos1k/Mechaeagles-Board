...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    // More strict check for high-end devices: require at least 8 cores for "high" quality
    const isPowerfulDevice = navigator.hardwareConcurrency && navigator.hardwareConcurrency >= 8;
    const isLowEndDevice = isMobile || (navigator.hardwareConcurrency && navigator.hardwareConcurrency <= 4);

    let effectiveQuality = quality;
    if (quality === 'high' && !isPowerfulDevice) effectiveQuality = 'medium';
    if (isLowEndDevice && effectiveQuality === 'high') effectiveQuality = 'medium';
    if (isMobile && effectiveQuality !== 'low') effectiveQuality = 'low';

    const qualitySettings = {
      low: { iterations: 16, waveIterations: 1, pixelRatio: 0.5, precision: 'mediump' as const, stepMultiplier: 2.0 },
      medium: { 
        iterations: 32, 
        waveIterations: 1, 
        pixelRatio: 0.75, 
    ...
    const targetFPS = effectiveQuality === 'high' ? 60 : 45;
    ...
    const animate = (currentTime: number) => {
      ...
      if (deltaTime >= frameTime) {
        ...
        // Add movement to the beam (oscillation)
        materialRef.current.uniforms.uOffset.value = Math.sin(t * 0.5) * 0.1;
